...
public class AuthController {
  ...
  @Operation(summary = "Refresh JWT token", description = "Refreshes the JWT token by generating a new one if old was a valid token")
  @ApiResponses({
   ...
   * 
   * @param loginRequest a DTO of the information that is sent such as username
   *                     and password
   */
  @Operation(summary = "Checks login credentials", description = "Checks login credentials and sends a mail to the registered mail address")
  @ApiResponses({
  ...
  /**
   * Checks wether the information given by the client is valid. If it is then it
   * will send
   * a two factor mail to the email provided by the user and register that user in
   * the db.
   *
   * @param body the user entity to be checked
   * @return {@code ResponseEntity}
   */
  @Operation(summary = "Register new user", description = "Creates a new user. Sends mail for authentication.")
  @ApiResponses({
      @ApiResponse(responseCode = "200", description = "User created", content = @Content(mediaType = "application/json", schema = @Schema(implementation = UserResponseDTO.class))),
      @ApiResponse(responseCode = "409", description = "Email or username already taken"),
  })
  @PostMapping("/register")
  public ResponseEntity<?> register(
      @Parameter(description = "User registration payload", required = true) @RequestBody UserRequestDTO body,
  ...
  /**
   * Completes the two-factor authentication process by validating the provided
   * code.
   *
   * @param request the two-factor authentication confirmation containing the code
   *                and the login information
   * @return a response entity indicating the result of the operation and cookies
   *         such as jwt and refresh token
   */
  @Operation(summary = "Confirm two-factor authentication", description = "Confirm the two-factor authentication process using the provided code.")
  @ApiResponses(value = {
   ...
   * 
   * @param body registration information such as name, username, password, email.
   *             Household id is not needed
   * @return statuscode 200 if the admin user was successfully created
   * @throws Exception
   */
  @Operation(summary = "Registers an admin user when logged in as a super admin", description = "A logged in superadmin can create another admin by sending registration information")
  @ApiResponses(value = {
